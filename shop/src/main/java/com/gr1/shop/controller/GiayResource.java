...
import com.gr1.shop.service.GiayQueryService;
...
/**
 * REST controller for managing {@link com.gr1.shop.domain.Giay}.
 */
@RestController
@RequestMapping("/api")
public class GiayResource {
    ...
    public GiayResource(GiayService giayService, GiayRepository giayRepository, GiayQueryService giayQueryService) {
     ...
     */
    @PutMapping("/giays/{id}")
    public ResponseEntity<GiayDTO> updateGiay(@PathVariable(value = "id", required = false) final String id, @RequestBody GiayDTO giayDTO)
        throws URISyntaxException {
        log.debug("REST request to update Giay : {}, {}", id, giayDTO);


        GiayDTO result = giayService.update(giayDTO);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, giayDTO.getId()))
            .body(result);
    }
